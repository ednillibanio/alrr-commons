...
public class EnderecoService implements EnderecoLocal, Serializable {
	...
	public Endereco preencherEnderecoPorCep(Cep cep) {
		...
		if (cep != null) {
			...
			// FIXME
			/*
			 * Municipio loc = localidadeBean.buscarPorUf(cep.getMunicipio().getUf(),
			 * cep.getMunicipio()); end.setMunicipio(loc); end.setCep(cep.getNumero());
			 * end.setLogradouro(cep.getLogradouro()); if
			 * (StringUtils.isNotBlank(cep.getBairro())) { bairro =
			 * bairroBean.buscarPorLocalidadeId(loc.getId(), cep.getBairro());
